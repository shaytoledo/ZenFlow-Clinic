...
| # | Task | Status | Date | Commit | Notes |
|---|---|---|---|---|---|
| 4.1a | Split `treatment.html` into partials + ordered scripts + one stylesheet | [x] | 2026-09-17 | | pure move: 1,967 → 66 lines; 10 partials, 10 classic scripts (shared globals, main.js last), `static/css/treatment.css`; the one Jinja value inside JS became a JSON island. Before/after DOM snapshot of 4 sessions (tag, id, class, style, attributes, text, 60 computed styles per element): identical |
