...
| 4.2a | Point cards: research, design tokens, card anatomy, selection model, a11y, RTL | [x] | 2026-09-17 | _pending_ | item 1a. Research + design in `docs/POINT_CARD_DESIGN.md` (WHO naming, Deadman entry layout, pregnancy cautions kept on the card face). `css/tokens.css` (ADR-25): `--zf-*` scales, semantic colours, channel palette, opt-in dark theme; every text pair ≥ 4.5:1 in both themes (test). New card: code badge → name → toggle → channel chip (name always shown) → caution → one-line location → "for this patient" → `<details>`. Cards and chips toggle a point (`aria-pressed`, "Added" text, running count, undo for any removal); tag label opens the point panel (was dead code; now a labelled dialog, Escape closes, focus returns). Logical properties only (test). Fixed on the way: Hebrew cards were all grey (theme looked up by the translated channel), WHO `KI3` found no data (table uses `KD3`), small text below AA. Browser-checked: interactions, tablet width, RTL, dark tokens |
| 4.2b | Point states: empty, skeleton loading, partial (batch A), failed; density toggle saved per therapist | [ ] | | | item 1a; replaces the timed progress bar |
| 4.2c | Responsive shell + print stylesheet + Playwright visual snapshots (3 widths × LTR/RTL × light/dark) | [ ] | | | item 1a. The shell is not responsive: the 232px sidebar never collapses, so below ~600px the page content is < 100px wide (4.2a only stacks the treatment page's two columns under 900px). Playwright needs its browser download |
