...
| 5.2 | Typed 409 `google_not_connected` error contract | [x] | 2026-09-17 | _pending_ | item 2. `send-recommendations` answers **409** `{code: google_not_connected, reason, title, message, action_url, connect_url[, text]}` (localised) instead of 200 `no_smtp`, both for the email send and for a 24h queue of an email-only patient (a send bound to fail). `EmailNotConfigured.reason` / `EmailSendError.token_invalid` (401 / `invalid_grant` / refused refresh); Google unreachable is a retryable 502, never a reconnect. `google_connection()` (never raises; `null` when unknown) feeds `/api/gmail-status` and the page bootstrap. The reconnect alert is created once and resolved by reconnecting or any successful send. `/auth/login?next=` returns to the page with `?google=connected|cancelled`; `next` must be a same-site path (open-redirect probes incl. `/\evil`, tab, CRLF, length). Settings card has `#google`. 9 en/he strings. Client reads the new contract (copy fallback kept; dialog comes in 5.3). Tests: 13 integration + 16 security (incl. Gmail base64 MIME, revoked token → one alert, network error ≠ reconnect) |
