...
| 4.3d | Image sourcing shortlist with licences (→ Q4) + lightbox with attribution and placeholder | [x] | 2026-09-17 | _pending_ | item 9. `docs/POINT_IMAGE_SOURCING.md`: 5 sources with licences checked at the source (own SVG set recommended; HOPE per-point diagrams CC BY-SA 4.0; Wikimedia per-file; BodyParts3D CC BY-SA 2.1 JP as a 3D base; WHO 2008 reference only), StatPearls excluded (CC BY-NC-ND); nothing downloaded. The side panel became a native `<dialog>` lightbox opened from a card's code badge or a tag: names in three scripts, image + thumbnails + zoom, attribution with `rel=license`, links limited to http(s) or `/media/`, placeholder when absent or broken, caution/location/actions/AI rationale. The old panel's code and styles are gone. Tests: node (placeholder, attribution, unsafe links, escaping, thumbnails, Hebrew) + e2e (open from badge and tag, focus, Escape, image from an ingest with zoom, 2 new snapshots — which caught the dialog pinned to a corner by the global margin reset) |
