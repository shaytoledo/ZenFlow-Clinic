...
| 4.2e | Playwright visual snapshots (3 widths × LTR/RTL × light/dark) | [x] | 2026-09-17 | _pending_ | item 1a. `tests/e2e/` (markers `e2e`, `slow`): the real app on a local port in a thread (same test DB and fakeredis), driven by the installed Chrome through Playwright (`ZF_E2E_BROWSER`, no browser download; skips without one). 12 snapshots of the AI points section + the shell at each width + the open phone drawer, compared with per-platform baselines (Pillow, 0.2% tolerance; a single badge colour change is caught at 0.76%). Repeatable: web fonts blocked, motion off, clock hidden, fixed data. `ZF_UPDATE_SNAPSHOTS=1` rewrites them. Dev deps `playwright`, `pillow` (lock: only the new pins). The snapshots showed the section header crowding on phones → it wraps now |
