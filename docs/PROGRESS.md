...
| 2.3 | State persistence across restarts | [x] | 2026-09-16 | | `bot/persistence.py::SqlitePersistence` (JSON rows in `bot_persistence`, not pickle); whitelisted scheduling keys only; flows idle past the timeout are not resumed; acceptance test drives a real `Application` offline through book → restart → finish. ADR-22 |
