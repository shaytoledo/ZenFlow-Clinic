...
| 4.3a | `acupoints` table + JSON seed + `python -m zenflow.seed` + `/api/acupoints`; page stops shipping the literal | [x] | 2026-09-17 | _pending_ | item 9. 26 points under WHO codes (KD3→KI3 alias; YIN folded into YINTANG), Han characters + English names added, `contraindications` replaces the JS pregnancy list (BL60/BL67/CV3/CV4 given reference rows), Hebrew in `translations`. init_db seeds an empty table; the CLI upserts idempotently and keeps other rows; the seed is validated (duplicate codes/aliases, unknown contraindications, missing fields). `GET /api/acupoints` (auth, page language, English channel for themes, alias map, ETag + 304, private cache). point-info.js now loads it before the cards render. Browser-checked EN + HE |
