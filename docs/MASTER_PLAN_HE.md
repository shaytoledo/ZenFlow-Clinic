...
> **כל משימה מסתיימת בענף דחוף (push) וב-Pull Request אל `master`** (ראה 1.1 מסירה ו-1.2).
> מעדכנים את `docs/PROGRESS.md` בסוף כל משימה, כולל קישור ל-PR.
>
...
מסירה     → דחוף (push) את הענף ופתח Pull Request אל `master`. כל משימה (N.x) מסתיימת ב-PR;
            משימות שהסתיימו באותו יום יכולות לחלוק PR אחד, PR לעולם לא חוצה שני שלבים.
            גוף ה-PR = מה ולמה, פקודות הדגמה, פלט השער המודבק (טסטים / lint / type /
            pre-commit), הערות rollback, ושורות PROGRESS.md שהוא סוגר. האדם סוקר וממזג;
            הסוכן לעולם לא ממזג PR של עצמו ולעולם לא דוחף ל-`master`.
            רשום את קישור ה-PR בתא ההערות של המשימה ב-docs/PROGRESS.md.
```

ענפים: `claude/<phase>-<task>-<slug>` (למשל `claude/0.5-security-triage`). בזמן ש-PR נמצא
בסקירה, המשימה הבאה מסתעפת ממנו (stacked PRs); אחרי שה-PR הבסיסי ממוזג, בצע rebase או merge
מ-`master` כדי שה-PR המוערם יציג רק את ה-diff שלו. force-push רק לענף לא-ממוזג שלך, לעולם לא ל-`master`.
...
- **כל משימה נמסרת כ-PR אל `master`.** אין push ישיר ל-`master`; הסוכן פותח את ה-PR, האדם ממזג.
  משימה בלי קישור PR ב-PROGRESS.md אינה גמורה.
...
9.11 נתונים בתנועה — כל מה שחוצה את האינטרנט או גבול בין מכונות (דרישת הבעלים, ADR-14)
    - HTTPS בלבד לדשבורד, ל-OAuth callbacks ולכל webhook עתידי; TLS 1.2+; HSTS (9.4);
      הפניה HTTP→HTTPS; תעודות מ-ACM / Let's Encrypt עם חידוש אוטומטי.
    - כל קריאה יוצאת — Telegram Bot API, Google Calendar/Gmail, Anthropic, Ollama כשהוא מרוחק —
      דרך https:// עם אימות תעודה דלוק (בדיקת grep: אין `verify=False`, אין `ssl.CERT_NONE`,
      אין http:// רגיל למארח שאינו מקומי; חוק ruff/bandit + טסט).
    - Redis דרך TLS עם AUTH (`rediss://`) בכל מקרה שאינו localhost; היסטוריית הרילי
      והיסטוריית ה-LLM שהוא מחזיק הן מידע קליני.
    - תעבורת בוט ⇄ web ⇄ worker (קובץ SQLite, Redis, ה-booking API העתידי) נשארת ברשת פרטית
      או ב-TLS; ה-API הפנימי (7.3) מאמת לקוחות-מכונה.
    - הודעות מטופל ⇄ מטפל עוברות ב-TLS של טלגרם לשרתי טלגרם; תעד שה-Bot API אינו מוצפן
      מקצה-לקצה ומה זה אומר על הסכמה (Q5). לעולם אל תשים תוכן הודעה, טוקנים או מזהים
      ב-URL, ב-query string או בלוגים.
    - גיבויים וייצואים מוצפנים לפני שהם עוזבים את המארח (9.9).
    טסטים: טסט אינוונטר של URL-ים יוצאים שנכשל על כל URL לא-מקומי שאינו https; טסט ולידציה
    ל-URL של Redis; טסט לכותרות CSP/HSTS; טסט להסתרת גוף הודעות בלוגים.
```
