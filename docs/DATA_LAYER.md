...
| `zenflow:alerts:{therapist_id}` | **Legacy**: the dashboard's old manual follow-up list. Nothing writes it; `GET /api/my/alerts` reads the `followups` table since Phase 6.4 and `DELETE` only clears this key | — | `web/routers/api/system.py` (delete only) | explicit delete |
