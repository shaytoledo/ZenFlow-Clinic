...
async def change_therapist(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    def _load_active():
        ...
        rows = (
            get_db()
            .execute("SELECT id, name FROM therapists WHERE active=1 ORDER BY name")
            .fetchall()
        )
    ...
    header = (
        t("bot_choose_therapist", lang) if len(active) > 1 else t("bot_available_therapists", lang)
    )
    ...
    if len(active) == 1:
        ...
        if update.message:
            await update.message.reply_text(
                text, parse_mode="Markdown", reply_markup=get_main_keyboard(lang)
            )
        elif update.callback_query:
            await update.callback_query.answer()
            await update.callback_query.edit_message_text(
...
async def back_to_main(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await query.edit_message_text(
        t("bot_what_to_do_menu", lang), reply_markup=get_main_keyboard(lang)
    )
    return SELECTING
