...
function pointCardHtml(pt, selected) {
  ...
  return `<article class="pc ${themeClass}${selected ? ' is-selected' : ''}" data-point-card="${escHtml(code)}"
    ...
    <header class="pc-head">
      <button type="button" class="pc-code" data-action="open-point-lightbox" data-code="${escHtml(code)}" title="${escHtml(pointText('showInfo', { code }))}" aria-haspopup="dialog">${escHtml(code)}</button>
